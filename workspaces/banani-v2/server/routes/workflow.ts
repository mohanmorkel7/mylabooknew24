  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!projectData.name || !projectData.assigned_team) {
        return res
          .status(400)
          .json({ error: "Missing required fields: name, assigned_team" });
      }
      ...
      if (await isDatabaseAvailable()) {
        ...
        const mockProject = {
          ...
          description:
            ...
            `Product development project created from completed lead ${leadId}`,
          source_type: "lead",
          source_id: leadId,
          project_type: "product_development",
          status: "created",
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!status) {
        return res
          .status(400)
          .json({ error: "Missing required field: status" });
      }
      ...
      if (await isDatabaseAvailable()) {
        ...
        // Update mock data for testing
        const mockProject = WorkflowMockData.projects.find(
          (p) => p.id === projectId,
        );
        if (mockProject) {
